...
from graph_utils import apply_edge_sparsity_embedding, apply_node_op_type_embedding
...
class EdgeGATLayer(graphNetworkLayer):
    ...
    def update_edge_fn(self, edge_features, sender_node_features, receiver_node_features, global_features):
        ...
        updated_edge_features = jnn.leaky_relu(out)
    ...
    def update_node_fn(self, current_node_features, outgoing_edge_features, incoming_edge_features, global_features):
        ...
        updated_node_features = jnn.leaky_relu(out)
...
class EdgeGATNetwork(graphNetwork):
    ...
    def __init__(
        ...
        self.sparsity_embedding = eqx.nn.Embedding(21, sparsity_embedding_size, key=embed_key_1)
        self.output_layer = eqx.nn.Linear(node_feature_shapes[-1], 1, key=out_key)

        init_edge_feature_size = sparsity_embedding_size + 4
        init_node_feature_size = 1

        keys = jrand.split(key, len(edge_feature_shapes))
        self.gat_layers = []
        self.gat_layers.append(EdgeGATLayer(init_edge_feature_size, init_node_feature_size, edge_feature_shapes[0], node_feature_shapes[0], keys[0]))
